...
def papers100M():
    ...
    np.savez('papers100M_labels.npz',train_idx=train_idx,val_idx=val_idx,test_idx=test_idx,train_labels=train_labels,val_labels=val_labels,test_labels=test_labels)
    ...
    #save feats
    np.save('papers100M_feat.npy',feat)
    del feat
    gc.collect()
    
    print('making the graph undirected')
    data.edge_index=to_undirected(data.edge_index,data.edge_attr,data.num_nodes)
    ...
    print('Write PL')
    f2=open('papers100M_adj_pl.txt','wb')
    for i in PL:
        m=struct.pack('I',i)
        f2.write(m)
    f2.close()    
    ...
    #"save graph
    print('Write EL')
    f1=open('papers100M_adj_el.txt','wb')
    for i in EL_re:
        m=struct.pack('I',i)
        f1.write(m)
    f1.close()    
